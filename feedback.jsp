...
<%
try{
    Class.forName("com.mysql.jdbc.Driver"); // Java 8
    Connection con = DriverManager.getConnection(
        "jdbc:mysql://localhost:3306/db","root",""
    );

    Statement st = con.createStatement();
    ResultSet rs = st.executeQuery("SELECT * FROM fd");
...
<tr>
    ...
    <td><%=rs.getString("ros")%></td>
    <td><%=rs.getString("com")%></td>
    <td>
                <a href="../crud/updatall.jsp?type=feedback&id=<%=rs.getInt("id")%>"class="btn btn-primary btn-sm">Update</a>
                <a href="../crud/deleteall.jsp?type=feedback&id=<%=rs.getInt("id")%>"class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete?')">Delete</a>
            </td>
</tr>
<%
    }
